...
if uploaded_file:
    try:
        # Specify data types to avoid dtype warnings
        dtype_mapping = {
            'COMPANY_ID': 'int32',
            'COMPANY_NAME': 'str',
            'BANK_TRANSACTION_REFERENCE': 'str',
            'BANK_TRANSACTION_TYPE': 'str',
            'BANK_TRANSACTION_TOTAL_AMOUNT': 'float32',
            'LINE_ITEM_DESCRIPTION': 'str',
            'RECONCILITION_ACCOUNT_NAME': 'str'
        }
        
        # Load the dataset with specified dtypes and prevent warnings
        data = pd.read_csv(uploaded_file, dtype=dtype_mapping, low_memory=False)
        ...
        label_encoders = {}
        for column in ['COMPANY_NAME', 'BANK_TRANSACTION_REFERENCE',
                       'BANK_TRANSACTION_TYPE', 'LINE_ITEM_DESCRIPTION',
